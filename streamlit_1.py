...
def get_data_from_excel():
    ...
    df["hours"]= pd.to_datetime(df["Time"], format="%H:%M:%S")
...
# st.dataframe(df_selection)    (can make filter on selection)
...
print(df_selection["Product line"])
...
fig_product_sales= px.bar(sales_by_product_line, x="Total", y = sales_by_product_line.index, title = "<b>Sales by Product Line</b>")


#, color_discreate_sequence=["#0083B8"] * len(sales_by_product_line)



# Sales by hour [Bar Chart]

sales_by_hour = df_selection.groupby(by=["hours"]).sum()[["Total"]]
